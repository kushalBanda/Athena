...
export class TuiMainScreen extends TuiBase implements TUI {
  ...
  protected doRender(): void {
    ...
    // Must run before applyLineResets, which strips the cursor marker.
    ...
    // Termux changes height on software-keyboard toggle; redrawing there replays all history.
    ...
    // Content shrunk: clear the empty rows. Overlays need the padding, so skip then.
    if (
    ...
    // Render only firstChanged..lastChanged; rendering to the end causes flicker.
    ...
    // cursorRow tracks end of content; hardwareCursorRow tracks the real terminal cursor.
    this.cursorRow = Math.max(0, newLines.length - 1);
