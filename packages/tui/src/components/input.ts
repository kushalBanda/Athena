...
export class Input implements Component, Focusable {
  ...
  handleInput(data: string): void {
    // Bracketed paste is delimited by \x1b[200~ and \x1b[201~.
    ...
    // Kitty CSI-u encodes printable keys too; decode before the control-char check,
    // since CSI-u sequences contain \x1b and would otherwise be rejected.
    ...
    // Accept printable Unicode, reject C0 (0x00-0x1F), DEL (0x7F) and C1 (0x80-0x9F).
  ...
  private yankPop(): void {
    ...
    // The previously yanked text is still at the end of the ring before rotation.
  ...
  private handlePaste(pastedText: string): void {
    ...
    // A single-line input cannot hold newlines; tabs become spaces.
  ...
  render(width: number): string[] {
    ...
    // Zero-width marker before the fake cursor, so IME positioning tracks the caret.
    const marker = this.focused ? CURSOR_MARKER : "";

    const cursorChar = `\x1b[7m${atCursor}\x1b[27m`; // reverse video, then normal
