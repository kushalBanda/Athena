...
/**
 * Merges graphemes inside a paste marker into one atomic segment, so cursor movement,
 * deletion and wrapping treat the marker as a single unit. Only IDs in `validIds` merge.
 */
function segmentWithMarkers(
  ...
  for (const seg of baseSegments) {
    ...
    if (marker && seg.index >= marker.start && seg.index < marker.end) {
      // Inside a marker: emit the merged segment once, at the marker start.
...
/** A word-wrap chunk plus its position in the original line. */
...
/**
 * Splits a line at word boundaries, falling back to character-level wrapping for words
 * longer than `maxWidth`. `preSegmented` supplies paste-marker-aware graphemes.
 */
export function wordWrapLine(
  ...
  // Wrap opportunity: the position after the last whitespace, where a break is allowed.
  let wrapOppIndex = -1;
  let wrapOppWidth = 0;
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (currentWidth + gWidth > maxWidth) {
      if (wrapOppIndex >= 0 && currentWidth - wrapOppWidth + gWidth <= maxWidth) {
        ...
        // No usable wrap opportunity: force-break here.
        chunks.push({
    ...
    if (gWidth > maxWidth) {
      // An atomic segment wider than maxWidth is split visually only; it stays
      // atomic for cursor movement and editing.
    ...
    // Break after the last of a whitespace run, or at any CJK boundary.
...
export class Editor implements Component, Focusable {
  ...
  // Must match the wrapping width used by layoutText().
  ...
  // Pre-snap cursorCol, kept when the cursor snaps to an atomic segment start,
  // so the next vertical move can still resolve a visual column.
  ...
  /** Records a submitted prompt for up/down arrow navigation. */
  ...
  /** setText variant that leaves history-browsing state intact. */
  ...
  render(width: number): string[] {
    ...
    // With padding the cursor may overflow into it; without padding reserve a column.
    ...
    // The marker lets TUI place the hardware cursor for IME candidate windows.
    ...
    for (const layoutLine of visibleLines) {
      ...
      if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
        ...
        // Zero-width, emitted before the fake cursor for IME positioning.
        ...
        if (after.length > 0) {
          ...
          // Width is unchanged: the grapheme is replaced, not added.
  ...
  handleInput(data: string): void {
    ...
    if (this.jumpMode !== null) {
      // Pressing the hotkey again cancels the jump.
      if (
    ...
    // Ctrl+C is handled by the parent (exit/clear).
    ...
    if (kb.matches(data, "tui.input.submit")) {
      ...
      // For terminals with no Shift+Enter: a trailing \ means newline, not submit.
    ...
    for (let i = 0; i < this.state.lines.length; i++) {
      ...
      if (lineVisibleWidth <= contentWidth) {
        ...
        for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
          ...
          // The cursor may sit in whitespace trimmed off the end of a chunk.
  ...
  /** Text with paste markers replaced by their stored content. */
  ...
  /** Programmatic insert at the cursor; atomic for undo. */
  ...
  /** Normalizes line endings to \n and expands tabs to 4 spaces. */
  private normalizeText(text: string): string {
    return text.replace(/\r\n/g, "\n").replace(/\r/g, "\n").replace(/\t/g, "    ");
  }

  /** Caller is responsible for undo snapshots and autocomplete; onChange fires once. */
  ...
  private insertCharacter(char: string, skipUndoCoalescing?: boolean): void {
    ...
    // Fish-style coalescing: a run of word chars is one undo unit; each space is its own.
    ...
    if (!this.autocompleteState) {
      ...
      // Trigger characters only count at a token boundary.
  ...
  private handlePaste(pastedText: string): void {
    ...
    // Some terminals re-encode control bytes in a paste as CSI-u Ctrl+<letter>; decode
    // them back so the filter below keeps newlines instead of leaking "[106;5u" text.
    ...
    // Separate a pasted path from an adjacent word character.
    ...
    // Large pastes are stored and replaced by a marker.
  ...
  private handleBackspace(): void {
    ...
    if (this.state.cursorCol > 0) {
      ...
      // Delete a whole grapheme, so emoji and combining marks stay intact.
      ...
      if (isPastedSegmented) {
        ...
        // Shift ids down in ascending order, not in marker order.
  ...
  /** Use for all non-vertical movement: clears the sticky column. */
  ...
  private moveToVisualLine(
    ...
    // Resolve a snapped cursor against its own visual line, so a resize cannot skew it.
    ...
    // Non-last segments clamp to length-1 to stay inside the segment.
    ...
    for (const seg of segments) {
      ...
      if (this.state.cursorCol < seg.index + seg.segment.length) {
        ...
        if (isContinuation && isMovingDown) {
          // Already visited this segment on the way down; skip its continuation lines.
        ...
        // Snap to the segment start; keep the pre-snap column for the next vertical move.
  ...
  /** Sticky-column rule: keep the preferred column while target lines are too short. */
  ...
  private deleteToStartOfLine(): void {
    ...
    if (this.state.cursorCol > 0) {
      ...
      // Backward deletion prepends to the kill ring.
      ...
      // Merge with the previous line; the newline counts as deleted text.
  ...
  private deleteToEndOfLine(): void {
    ...
    if (this.state.cursorCol < currentLine.length) {
      ...
      // Forward deletion appends to the kill ring.
      ...
      // Merge with the next line; the newline counts as deleted text.
  ...
  private deleteWordBackwards(): void {
    ...
    // At column 0 this behaves like backspace: merge with the previous line.
    if (this.state.cursorCol === 0) {
      ...
      // moveWordBackwards resets lastAction, so capture it first.
  ...
  private deleteWordForward(): void {
    ...
    // At end of line this deletes the newline, merging with the next line.
    if (this.state.cursorCol >= currentLine.length) {
      ...
      // moveWordForwards resets lastAction, so capture it first.
      const wasKill = this.lastAction === "kill";
  ...
  /** Maps each visual line to its logical line, start column and length. */
  ...
  private findVisualLineAt(
    ...
    for (let i = 0; i < visualLines.length; i++) {
      ...
      // On the last segment of a line the cursor may also sit at `length`.
  ...
  private moveCursor(deltaLine: number, deltaCol: number): void {
    ...
    if (deltaCol !== 0) {
      ...
      if (deltaCol > 0) {
        // Move by a whole grapheme, so emoji and combining marks stay intact.
        if (this.state.cursorCol < currentLine.length) {
          const afterCursor = currentLine.slice(this.state.cursorCol);
          const graphemes = [...this.segment(afterCursor, "grapheme")];
          const firstGrapheme = graphemes[0];
          ...
          // Cannot move, but record the sticky column for later up/down moves.
          const currentVL = visualLines[currentVisualLine];
          if (currentVL) {
    ...
    // Cursor movement changes the text before the cursor, so an open picker is stale.
    if (this.autocompleteState) {
      this.updateAutocomplete();
    }
  }
  ...
  /** Replaces the just-yanked text with the previous kill ring entry. */
  ...
  /** The yanked text is killRing[end], because the ring has not rotated yet. */
  private deleteYankedText(): void {
    ...
    if (yankLines.length === 1) {
      ...
      // The cursor sits at the end of the last yanked line.
  ...
  /** Case-sensitive multi-line search that skips the current cursor position. */
  ...
  /** The slash menu is only available on the first line. */
  ...
  /** Exact value match wins, then first prefix match; -1 keeps the default highlight. */
