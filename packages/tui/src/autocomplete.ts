...
export interface SlashCommand {
  ...
  description?: string;
  argumentHint?: string;
  /** Returns null if this command has no argument completion. */
...
export interface AutocompleteProvider {
  ...
  /** Returns null if no suggestions are available at the cursor. */
  getSuggestions(
    lines: string[],
    cursorLine: number,
    cursorCol: number,
  ...
  /** Applies the selected item, returning new text and cursor position. */
  applyCompletion(
    lines: string[],
    cursorLine: number,
    cursorCol: number,
...
/** Handles both slash-command and file-path completion. */
export class CombinedAutocompleteProvider implements AutocompleteProvider {
  ...
  applyCompletion(
    ...
    // A slash command starts the line and has no path separator after the leading "/".
    ...
    if (prefix.startsWith("@")) {
      // No trailing space after directories, so the user can keep completing deeper.
    ...
    // Slash-command argument context: "/command <arg>".
    const textBeforeCursor = currentLine.slice(0, cursorCol);
  ...
  private extractPathPrefix(text: string, forceExtract: boolean = false): string | null {
    ...
    // Tab always forces a completion attempt.
    if (forceExtract) {
      return pathPrefix;
    }
    ...
    // Empty text must not trigger suggestions; only an empty token after a space does.
  ...
  private expandHomePath(path: string): string {
    if (path.startsWith("~/")) {
      ...
      // Preserve the trailing slash; callers use it to detect a directory prefix.
  ...
  private getFileSuggestions(prefix: string): AutocompleteItem[] {
    try {
      ...
      for (const entry of entries) {
        ...
        // A symlink to a directory must also count as a directory.
  ...
  /** Higher score = better match; directories get a bonus so folders sort first. */
  ...
  /** Fuzzy file search via fd, which respects .gitignore. */
  ...
  shouldTriggerFileCompletion(lines: string[], cursorLine: number, cursorCol: number): boolean {
    ...
    // A slash command being typed at line start is not a file path.
