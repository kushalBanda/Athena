/**
 * Keyboard input handling: legacy terminal sequences plus Kitty keyboard protocol.
 * https://sw.kovidgoyal.net/kitty/keyboard-protocol/
 *
 * Some ctrl+symbol combos overlap with ASCII codes (e.g. ctrl+[ = ESC) and can only
 * be used in ctrl+shift form.
 */
...
/** Union of all valid key identifiers; catches typos at compile time. */
export type KeyId = BaseKey | ModifiedKeyId<BaseKey>;

/** Typed key identifier builders, e.g. Key.escape, Key.ctrl("c"), Key.ctrlShift("p"). */
export const Key = {
  // Special keys
...
/** Kitty keyboard protocol flag 2 event types: 1=press, 2=repeat, 3=release. */
...
/** Only meaningful when Kitty keyboard protocol flag 2 is active. */
export function isKeyRelease(data: string): boolean {
  // Paste content can contain ":3F" (e.g. MAC addresses), so exclude pastes first.
  if (data.includes("\x1b[200~")) {
    return false;
  }

  // Release events with flag 2 contain ":3": \x1b[<codepoint>;<modifier>:3u
  if (
    data.includes(":3u") ||
    data.includes(":3~") ||
    data.includes(":3A") ||
...
export function isKeyRepeat(data: string): boolean {
  // Paste content can contain ":2F"; see isKeyRelease().
  if (data.includes("\x1b[200~")) {
    return false;
  }
...
function parseKittySequence(data: string): ParsedKittySequence | null {
  // CSI-u: \x1b[<codepoint>[:<shifted>[:<base>]][;<mod>[:<event>]]u
...
function matchesKittySequence(
  ...
  // Base layout key lets Ctrl+С (Cyrillic) match Ctrl+c. Only used when the codepoint
  // is not itself a known Latin letter/symbol, else remapped layouts (Dvorak, Colemak)
  // would false-match on physical key position.
...
/** xterm modifyOtherKeys: CSI 27 ; modifiers ; keycode ~ (modifiers 1-indexed). */
...
/** Raw 0x08 is Ctrl+Backspace on Windows Terminal but plain Backspace elsewhere. */
...
/** Control character for a key: code & 0x1f. Letters a-z map to 1-26, [\]_ to 27-31. */
function rawCtrlChar(key: string): string | null {
  ...
  // "-" and "_" are the same physical key on US keyboards.
  if (char === "-") {
...
/** Match raw terminal input against a key identifier such as "ctrl+c" or "shift+tab". */
export function matchesKey(data: string, keyId: KeyId): boolean {
  ...
  switch (key) {
    ...
    case "return":
      if (modifier === MODIFIERS.shift) {
        ...
        // Under Kitty protocol these are custom terminal mappings:
        // \x1b\r from Kitty's shift+enter send_text, \n from Ghostty's keybind.
        if (_kittyProtocolActive) {
      ...
      if (modifier === MODIFIERS.alt) {
        ...
        // \x1b\r is alt+enter only in legacy mode; Kitty sends CSI-u instead.
        if (!_kittyProtocolActive) {
          return data === "\x1b\r";
        }
        return false;
  ...
  if (key.length === 1 && ((key >= "a" && key <= "z") || isDigitKey(key) || SYMBOL_KEYS.has(key))) {
    ...
    if (modifier === MODIFIERS.ctrl + MODIFIERS.alt && !_kittyProtocolActive && rawCtrl) {
      // Legacy ctrl+alt+key is ESC + control char; fall through so tmux CSI-u still matches.
...
function formatParsedKey(
  ...
  // Base layout key is only used for unrecognized codepoints; otherwise remapped
  // layouts (Dvorak, Colemak) would report the QWERTY physical key name.
...
export function parseKey(data: string): string | undefined {
  ...
  // Under Kitty protocol, \x1b\r and \n are shift+enter terminal mappings, not alt+enter.
  if (_kittyProtocolActive) {
    if (data === "\x1b\r" || data === "\n") return "shift+enter";
  }
...
/**
 * Extract a printable character from a Kitty CSI-u sequence.
 * With flag 1 (disambiguate) active, terminals send CSI-u even for plain characters.
 * Only plain or Shift-modified keys are accepted; Ctrl/Alt go through keybinding matching.
 */
export function decodeKittyPrintable(data: string): string | undefined {
  ...
  // CSI-u modifiers are 1-indexed; normalize to our bitmask.
  const modifier = Number.isFinite(modValue) ? modValue - 1 : 0;

  // Reject unsupported modifier bits (e.g. Super) so modifier-only events insert nothing.
