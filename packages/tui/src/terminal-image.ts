...
// Updated by the TUI when the terminal answers the cell-size query.
...
/** tmux only re-emits OSC 8 when `client_termfeatures` lists `hyperlinks`. */
...
export function detectCapabilities(
  ...
  // WT_SESSION isn't always set (e.g. cmd.exe launched from Win+R).
  if (isWindowsConsole) {
    return { images: null, trueColor: true, hyperlinks: false };
  }

  // Unknown terminal: terminals that swallow OSC 8 drop the URL entirely.
  return { images: null, trueColor: hasTrueColorHint, hyperlinks: false };
}
...
export function isImageLine(line: string): boolean {
  ...
  // Multi-row images carry a cursor-up prefix, so the sequence isn't at index 0.
  return line.includes(KITTY_PREFIX) || line.includes(ITERM2_PREFIX);
}

/** Random Kitty image ID, avoiding collisions across module instances. */
...
/** Delete a Kitty image by ID; uppercase 'I' also frees the image data. */
export function deleteKittyImage(imageId: number): string {
  return `\x1b_Ga=d,d=I,i=${imageId},q=2\x1b\\`;
}

/** Delete all visible Kitty images; uppercase 'A' also frees the image data. */
...
/** Wrap text in an OSC 8 hyperlink; unsupporting terminals show plain text. */
