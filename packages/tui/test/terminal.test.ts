...
describe("ProcessTerminal Kitty keyboard protocol negotiation", () => {
  ...
  // Bun's node:test shim doesn't implement mock.timers yet (as of Bun 1.3.4) — skip until supported.
  it.skip("tracks split Kitty confirmation", () => {
  ...
  // Bun's node:test shim doesn't implement mock.timers yet (as of Bun 1.3.4) — skip until supported.
  it.skip("replays buffered CSI-prefix input when it is not a Kitty response", () => {
    mock.timers.enable({ apis: ["setTimeout"] });
    const harness = setupNegotiation();
    try {
      harness.send("\x1b[");
