...
describe("viewport layout", () => {
  ...
  it("omits gaps around invisible entries", () => {
    const stack = new VStack(
      [
        new Text("one", 0, 0),
        { component: new Text("hidden", 0, 0), visible: () => false },
        new Text("two", 0, 0),
      ],
      { gap: 1 },
    );
  ...
  it("renders a transient proportional scrollbar without replacing cell content", async () => {
    ...
    const content = new Text(
      ...
      0,
      0,
      (text) => `${contentBackground}${text}\x1b[49m`,
    );
    ...
    assert.ok(
      lines[1]!.lastIndexOf(contentBackground) < lines[1]!.lastIndexOf(scrollbarBackground),
    );
    ...
    assert.ok(
      renderLayoutFrame(automatic, 6, 4, () => {}).lines.every(
        (line) => !line.includes(scrollbarBackground),
      ),
    );
    ...
    assert.strictEqual(
      alwaysOverflowingFrame.lines.filter((line) => line.includes(scrollbarBackground)).length,
      2,
    );

    const thumbHeightFor = (contentHeight: number) => {
      const sized = new ScrollView(
        new Text(Array.from({ length: contentHeight }, () => "x").join("\n"), 0, 0),
        {
          scrollbar: "auto",
          scrollbarStyle,
        },
      ...
      return renderLayoutFrame(sized, 6, 20, () => {}).lines.filter((line) =>
        line.includes(scrollbarBackground),
      ).length;
    };
  ...
  it("updates reserved scrollbar layout at runtime", () => {
    ...
    const render = () =>
      renderLayoutFrame(new HStack([scrollView], { align: "start" }), 6, 2, () => {});
    const always = render();
  ...
  it("measures nested scroll content from constrained child geometry", () => {
    ...
    const outer = new ScrollView(
      new VStack([{ component: inner, basis: 2 }, new Text("tail", 0, 0)]),
    );
