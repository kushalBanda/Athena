...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as ATHENA_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.ATHENA_SESSION_ID = metadata.id;
						execution.env.ATHENA_SESSION_FILE = sessionFile ?? "";
						execution.env.ATHENA_PROVIDER = model.provider;
						execution.env.ATHENA_MODEL = model.id;
						execution.env.ATHENA_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
