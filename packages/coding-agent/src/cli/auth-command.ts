...
export function printAuthCommandHelp(): void {
	console.log(`Usage:
  athena auth print-api-key [--provider <provider>] [--model <model>]
  athena auth print-bearer-token [--provider <provider>] [--model <model>] [--min-expiry <duration>]
  athena auth check [--provider <provider>] [--model <model>] [--json] [--credentials] [--no-refresh]
