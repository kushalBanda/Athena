...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@kushalbanda/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@kushalbanda/coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown athena and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Athena loaded without re-discovering resources. */
...
export interface ExtensionAPI {
	...
	/** Register a transformer for user and assistant Markdown before Athena renders it in the interactive transcript. */
...
/** Configuration for registering a provider via athena.registerProvider(). */
...
export type ExtensionFactory = (athena: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for athena.* API methods.
