...
export default function (athena: ExtensionAPI) {
	...
	athena.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	athena.on("turn_end", async (_event, ctx) => {
	...
	athena.registerCommand("rpc-input", {
	...
	athena.registerCommand("rpc-editor", {
	...
	athena.registerCommand("rpc-prefill", {
