...
async function resolveGitHubRepo(athena: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await athena.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (athena: ExtensionAPI): void {
	athena.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(athena, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await athena.exec(
					"gh",
					[
						"issue",
						"list",
