...
async function checkDirtyRepo(
	...
	const { stdout, code } = await athena.exec("git", ["status", "--porcelain"]);
...
export default function (athena: ExtensionAPI) {
	athena.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(athena, ctx, action);
	});

	athena.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(athena, ctx, "fork");
	});
}
