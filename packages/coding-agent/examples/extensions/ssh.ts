...
export default function (athena: ExtensionAPI) {
	athena.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	athena.on("session_start", async (_event, ctx) => {
		...
		const arg = athena.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
