...
export default function (athena: ExtensionAPI) {
	athena.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await athena.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[athena] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await athena.exec("git", ["add", "-A"]);
		const { code: commitCode } = await athena.exec("git", ["commit", "-m", commitMessage]);
