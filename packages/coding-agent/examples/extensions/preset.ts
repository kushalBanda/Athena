...
export default function presetExtension(athena: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: athena.getThinkingLevel(),
				tools: athena.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			athena.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = athena.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				athena.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await athena.setModel(originalState.model);
				}
				athena.setThinkingLevel(originalState.thinkingLevel);
				athena.setActiveTools(originalState.tools);
			} else {
				athena.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	athena.registerShortcut(Key.ctrlShift("u"), {
	...
	athena.on("before_agent_start", async (event) => {
	...
	athena.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = athena.getFlag("preset");
	...
	athena.on("turn_start", async () => {
		if (activePresetName) {
			athena.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
