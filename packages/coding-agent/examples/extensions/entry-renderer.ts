 ...
 * to the LLM. Custom entries are stored in the session via athena.appendEntry() and
 * rendered in interactive mode via athena.registerEntryRenderer().
 *
...
export default function (athena: ExtensionAPI) {
	athena.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	athena.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			athena.appendEntry<StatusCardData>("status-card", {
