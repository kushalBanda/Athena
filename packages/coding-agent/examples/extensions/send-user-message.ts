 ...
 *
 * Demonstrates athena.sendUserMessage() for sending user messages from extensions.
 * Unlike athena.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (athena: ExtensionAPI) {
	...
	athena.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			athena.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	athena.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				athena.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	athena.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				athena.sendUserMessage(args, { deliverAs: "followUp" });
	...
	athena.registerCommand("askwith", {
