...
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@kushalbanda/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@kushalbanda/tui";
...
export default function (athena: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like athena-doom would need)
	athena.registerCommand("overlay-animation", {
	...
	athena.registerCommand("overlay-anchors", {
	...
	athena.registerCommand("overlay-margins", {
	...
	athena.registerCommand("overlay-stack", {
	...
	athena.registerCommand("overlay-overflow", {
	...
	athena.registerCommand("overlay-edge", {
	...
	athena.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	athena.registerCommand("overlay-maxheight", {
	...
	athena.registerCommand("overlay-sidepanel", {
	...
	athena.registerCommand("overlay-toggle", {
	...
	athena.registerCommand("overlay-passive", {
	...
	athena.registerCommand("overlay-focus", {
	...
	athena.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/athena-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/athena-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like athena-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (athena-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
