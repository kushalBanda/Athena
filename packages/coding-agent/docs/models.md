...
`samplingParams` is a free-form object merged verbatim into every request body for the model, after the fields athena sets itself, so its keys win. Use it to send sampling parameters athena does not model — including server-specific ones like llama.cpp's `min_p` or vLLM's `top_k`:
...
Only OpenAI-compatible APIs apply it (`openai-completions`, `openai-responses`, `azure-openai-responses`); other APIs ignore it. Keys override athena's named request fields (for example a `temperature` key here beats the request-level temperature), so prefer it as the single source of sampling truth for a model. In `modelOverrides`, `samplingParams` merges per key with the base model's value.
...
| `chatTemplateArgs` | `chat_template_args` values for `thinkingFormat: "baseten"`; use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for athena-controlled thinking values |
