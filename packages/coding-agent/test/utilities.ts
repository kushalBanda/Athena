...
import type { OAuthCredentials } from "@kushalbanda/ai";
import { getModel, streamSimple } from "@kushalbanda/ai/compat";
import { builtinProviders } from "@kushalbanda/ai/providers/all";
...
/** Path to the real athena agent config directory */
export const ATHENA_AGENT_DIR = join(homedir(), ".athena", "agent");
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `athena-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
