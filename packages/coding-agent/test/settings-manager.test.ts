...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates athena starting up)
	...
	describe("project settings directory creation", () => {
		it("should not create .athena folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .athena folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .athena folder that beforeEach created
			rmSync(join(projectDir, ".athena"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .athena folder should NOT have been created just from reading
		...
		it("should create .athena folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .athena folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .athena folder should NOT exist yet
			expect(existsSync(join(projectDir, ".athena"))).toBe(false);
			...
			// Now .athena folder should exist
	...
	describe("defaultTools", () => {
		it("loads global defaults and lets project settings replace them", () => {
			...
			writeFileSync(join(projectDir, ".athena", "settings.json"), JSON.stringify({ defaultTools: ["grep"] }));
	...
	describe("mcpServers", () => {
		it("reads user-declared MCP servers, project overriding global per field", () => {
			writeFileSync(
				join(agentDir, "settings.json"),
				JSON.stringify({ mcpServers: { acme: { type: "http", url: "https://global.example/mcp" } } }),
			);
			writeFileSync(
				join(projectDir, ".athena", "settings.json"),
				JSON.stringify({ mcpServers: { acme: { oauth: true }, local: { type: "stdio", command: ["local-mcp"] } } }),
			);
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getMcpServers()).toEqual({
				acme: { type: "http", url: "https://global.example/mcp", oauth: true },
				local: { type: "stdio", command: ["local-mcp"] },
			});
		});

		it("is undefined when no servers are configured", () => {
			const manager = SettingsManager.create(projectDir, agentDir);
