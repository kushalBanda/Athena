...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `athena-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(athena) => {
				athena.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(athena) => {
				athena.on("session_start", () => {
					athena.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native athena-ai providers during extension loading", async () => {
		const session = await createSession([
			(athena) => {
				athena.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(athena) => {
				athena.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						athena.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native athena-ai providers at command time", async () => {
		const session = await createSession([
			(athena) => {
				athena.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						athena.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
