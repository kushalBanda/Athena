import { type AuthType, type CredentialStore, InMemoryCredentialStore } from "@kushalbanda/ai";
...
describe("ModelRuntime auth options", () => {
	it("accepts a athena-ai CredentialStore", async () => {
	...
	it("projects provider-owned methods, names, and status", async () => {
		...
		expect(options).toEqual(
			expect.arrayContaining([
				...
				expect.objectContaining({
					type: "oauth",
					provider: expect.objectContaining({ id: "anthropic", name: "Claude Code" }),
				}),
				expect.objectContaining({
					type: "api_key",
