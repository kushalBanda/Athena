...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "athena-skills", "browser-tools");
...
export default function(athena) {
	...
	athena.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(athena) {
	athena.registerCommand("project-trusted", {
	...
	athena.registerCommand("project-only", {
	...
	athena.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.athena", async () => {
