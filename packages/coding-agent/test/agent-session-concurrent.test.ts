...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `athena-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(athena) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = athena;
			},
			(athena) => {
		...
		athena!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
