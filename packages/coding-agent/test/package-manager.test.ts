...
Content`,
		...
		it("should auto-discover root markdown skills from .athena skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .athena", async () => {
...
Content`,
		...
		it("should resolve directory with package.json athena.extensions in extensions setting", async () => {
			// Create a package with athena.extensions in package.json
			...
			// Should find the extensions declared in package.json athena.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .athena/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-athena", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-athena"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-athena\ndescription: user athena\n---\n");
		...
		it("should use the project .athena dir as baseDir for project .athena skills", async () => {
			const projectBaseDir = join(tempDir, ".athena");
			const skillPath = join(projectBaseDir, "skills", "project-athena", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-athena"), { recursive: true });
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .athena auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".athena\n");
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with athena manifest", async () => {
		...
		it("should keep athena manifest entries with leading tilde package-relative", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should install user npm packages into the athena-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "athena-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .athena settings base", () => {
	...
	describe("pattern filtering in athena manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "athena-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "athena-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:athena-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:athena-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json athena.extensions manifest in subdirectories", async () => {
