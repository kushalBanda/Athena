...
describe("no-builtin-tools keeps extension tools enabled", () => {
	let tempDir: string;
	let agentDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `athena-no-builtin-tools-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("keeps extension tools active when built-in defaults are disabled", async () => {
		...
		expect(
			session
				...
				.map((tool) => tool.name)
				.sort(),
		).toEqual(["bash", "dynamic_tool", "edit", "find", "grep", "ls", "read", "websearch", "write"]);
