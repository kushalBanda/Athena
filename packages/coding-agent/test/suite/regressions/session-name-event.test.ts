...
describe("session name changes emit an event", () => {
	...
	it("emits session_info_changed when an extension calls athena.setSessionName", async () => {
	...
	it("emits session_info_changed to extensions", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(athena) => {
					api = athena;
					athena.on("session_info_changed", (event) => {
