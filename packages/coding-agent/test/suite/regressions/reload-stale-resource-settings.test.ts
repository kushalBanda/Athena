...
import { registerFauxProvider } from "@kushalbanda/ai/compat";
...
describe("reload stale resource settings", () => {
	...
	it("applies updated top-level prompt settings on reload after startup", async () => {
		const tempDir = join(tmpdir(), `athena-2753-${Date.now()}-${Math.random().toString(36).slice(2)}`);
