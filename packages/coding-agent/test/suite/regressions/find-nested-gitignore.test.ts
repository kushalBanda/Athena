 ...
 * manual collection.
 */
describe("nested .gitignore rules leak into sibling directories", () => {
	let tempRoot: string;
	...
	describe("flat sibling case", () => {
		beforeEach(() => {
			tempRoot = mkdtempSync(join(tmpdir(), "athena-3303-flat-"));
	...
	describe("deeply nested case", () => {
		beforeEach(() => {
			tempRoot = mkdtempSync(join(tmpdir(), "athena-3303-deep-"));
