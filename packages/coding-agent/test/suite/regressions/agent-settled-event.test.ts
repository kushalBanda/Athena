...
describe("agent settled event and idle waiting", () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(athena) => {
					...
					athena.on("agent_settled", (_event, ctx) => {
			...
			extensionFactories: [
				(athena) => {
					athena.on("agent_end", () => {
						...
						queuedFollowUp = true;
						athena.sendUserMessage("status follow-up", { deliverAs: "followUp" });
					});
					athena.on("agent_settled", (_event, ctx) => {
	...
	it("extension command waitForIdle waits for session-level settlement", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(athena) => {
					athena.registerCommand("after-idle", {
