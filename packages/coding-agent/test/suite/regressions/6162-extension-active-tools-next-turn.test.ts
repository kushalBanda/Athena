import { fauxAssistantMessage, fauxToolCall } from "@kushalbanda/ai";
...
describe("extension active tools next-turn refresh", () => {
	it("applies athena.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(athena) => {
				athena.registerTool({
					...
					execute: async () => {
						athena.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(athena) => {
				athena.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						athena.setActiveTools([...athena.getActiveTools(), "after_load"]);
						return {
