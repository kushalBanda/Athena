...
describe("replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `athena-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale athena/ctx", async () => {
		const events: string[] = [];
		let oldCtx: ExtensionCommandContext | undefined;
		let oldPi: ExtensionAPI | undefined;
		let oldSessionFile: string | undefined;
		let staleCtxThrows = false;
		let staleAthenaThrows = false;
		...
		expect(staleAthenaThrows).toBe(true);
	...
	it("supports withSession for fork", async () => {
		const { runtime } = await createRuntimeForTest(
			(athena) => {
				athena.registerCommand("fork-it", {
					description: "fork-it",
					handler: async (_args, ctx) => {
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(athena) => {
				athena.registerCommand("switch-it", {
