...
import type { AgentMessage, AgentTool } from "@kushalbanda/agent-core";
import { Agent } from "@kushalbanda/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@kushalbanda/ai/compat";
import { registerFauxProvider, streamSimple } from "@kushalbanda/ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `athena-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
