...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@kushalbanda/ai";
...
describe("AgentSession prompt characterization", () => {
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `athena-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("dispatches extension commands without consuming a provider response", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(athena) => {
					athena.registerCommand("testcmd", {
	...
	it("throws when prompted during manual compaction", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(athena) => {
					athena.on("session_before_compact", async (event) => {
