...
import { Agent, type AgentEvent, type AgentTool } from "@kushalbanda/agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@kushalbanda/ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `athena-retry-test-${Date.now()}`);
