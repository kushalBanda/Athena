import { AthenaClient, type ByteTransport, type ByteTransportHandlers } from "@kushalbanda/client";
import {
	type ClientMessage,
...
export async function connectClient(server: MemoryServer): Promise<AthenaClient> {
	const client = new AthenaClient({ transportFactory: (handlers) => server.connect(handlers) });
