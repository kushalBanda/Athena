...
import {
	checkForNewAthenaVersion,
	comparePackageVersions,
	formatVersionCheckError,
	getLatestAthenaRelease,
	getLatestAthenaVersion,
...
describe("version checks", () => {
	...
	it("returns only newer versions", async () => {
		...
		await expect(checkForNewAthenaVersion("1.2.3")).resolves.toBeUndefined();
	...
	it("retries a transient version request when explicitly requested", async () => {
		...
		await expect(getLatestAthenaRelease("1.2.3", { retry: true })).resolves.toEqual({ version: "1.2.4" });
	...
	it("returns the active package metadata from the version check api", async () => {
		...
		await expect(getLatestAthenaRelease("1.2.3")).resolves.toEqual({
			packageName: "@new-scope/athena",
			version: "1.2.4",
		});
	});
	...
	it("returns update notes from the version check api", async () => {
		...
		await expect(getLatestAthenaRelease("1.2.3")).resolves.toEqual({ note: "**Read this**", version: "1.2.4" });
	});
