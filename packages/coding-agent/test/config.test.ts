...
const originalAthenaPackageDir = process.env.ATHENA_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalAthenaPackageDir === undefined) {
		delete process.env.ATHENA_PACKAGE_DIR;
	} else {
		process.env.ATHENA_PACKAGE_DIR = originalAthenaPackageDir;
	}
...
function createNpmPrefixInstall(template = "athena-prefix-"): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	const root = join(prefix, "lib", "node_modules");
	const scopeDir = join(root, "@athena");
...
function createBunGlobalInstall(): { packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "athena-bun-"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@athena+coding-agent@0.67.68\\node_modules\\@athena\\coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@kushalbanda/athena")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @kushalbanda/athena",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@kushalbanda/athena")).toBeUndefined();
		expect(getUpdateInstruction("@kushalbanda/athena")).toBe(
			"Update @kushalbanda/athena using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@kushalbanda/athena");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@kushalbanda/athena"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @kushalbanda/athena`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@kushalbanda/athena", undefined, {
			packageName: "@kushalbanda/athena",
			installSpec: "@kushalbanda/athena@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@kushalbanda/athena@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @kushalbanda/athena@1.2.3`,
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					...
					display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/athena`,
				},
			],
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@kushalbanda/athena", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		const { prefix } = createNpmPrefixInstall("athena prefix ");

		const command = getSelfUpdateCommand("@kushalbanda/athena");

		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @kushalbanda/athena`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@athenacoding-agent";
		process.env.ATHENA_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@kushalbanda/athena")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @kushalbanda/athena",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@kushalbanda/athena"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @kushalbanda/athena",
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/athena"],
					display: "pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/athena",
				},
			],
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		const temp = mkdtempSync(join(tmpdir(), "athena-pnpm11-"));
		const binDir = join(temp, "bin");
		const root = join(temp, "Library", "pnpm", "global", "v11");
		const packageName = "@kushalbanda/athena";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@athena", "coding-agent");
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/athena"],
					display: "yarn global add --ignore-scripts @new-scope/athena",
				},
			],
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/athena"],
					display: "bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/athena",
				},
			],
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@kushalbanda/athena")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@kushalbanda/athena")).toContain("the install path is not writable");
	});
});
