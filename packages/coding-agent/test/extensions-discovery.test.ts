...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "athena-ext-test-"));
	...
	const extensionCode = `
		export default function(athena) {
			athena.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("loads the coding-agent entrypoint without rewriting athena-ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "@kushalbanda/athena";
				void getAgentDir;
				export default function(athena) {
					athena.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only athena-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@kushalbanda/ai/oauth";
				void oauth;
				export default function(athena) {
					athena.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json athena field", async () => {
	...
	it("keeps package.json athena extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with athena field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without athena field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ athena: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(athena) {
				athena.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				athena.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				athena.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(athena) {
				athena.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(athena) {
				athena.on("agent_start", async () => {});
				athena.on("tool_call", async (event) => undefined);
				athena.on("agent_end", async () => {});
			}
		`;
	...
	it("loads extension with shortcuts", async () => {
		const extCode = `
			export default function(athena) {
				athena.registerShortcut("ctrl+t", {
