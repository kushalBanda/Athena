...
import { NodeExecutionEnv } from "@kushalbanda/agent-core/node";
...
const defaultPromptTools = [
	...
	createPromptTool("bash", "Execute bash commands (ls, grep, find, etc.)", [
		"You can inspect ATHENA_* environment variables for current model and session details.",
	]),
...
describe("coding-agent Harness construction", () => {
	...
	test("preserves coding-agent prompt snippets and guideline order", () => {
		...
		expect(prompt).toContain("You can inspect ATHENA_* environment variables for current model and session details.");
		expect(prompt.indexOf("Use read to examine files")).toBeLessThan(
			prompt.indexOf("You can inspect ATHENA_* environment variables"),
		);
	});
	...
	test("sets the optional session file in the default bash tool environment", async () => {
		...
		try {
			...
			expect(env.executionOverrides).toEqual({
				ATHENA_SESSION_ID: "session-file-harness",
				ATHENA_SESSION_FILE: "/sessions/current.jsonl",
				ATHENA_PROVIDER: "google",
				ATHENA_MODEL: "gemini-2.5-flash",
				ATHENA_REASONING_LEVEL: "high",
			});
	...
	test("keeps bash ATHENA model variables synchronized with Harness state", async () => {
		...
		try {
			...
			expect(env.executionOverrides).toEqual({
				ATHENA_SESSION_ID: "dynamic-bash-session",
				ATHENA_SESSION_FILE: "",
				ATHENA_PROVIDER: "anthropic",
				ATHENA_MODEL: "claude-sonnet-4-5",
				ATHENA_REASONING_LEVEL: "low",
			});
			expect(Object.hasOwn(env.executionOverrides ?? {}, "ATHENA_SESSION_FILE")).toBe(true);
			expect(env.executionOverrides?.ATHENA_SESSION_FILE).toBe("");
			expect(result.content).toEqual([
				{
					type: "text",
	...
	test("builds the default prompt from active tools and resolved prompt resources", () => {
		...
		expect(prompt).not.toContain("You can inspect ATHENA_* environment variables");
