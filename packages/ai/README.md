...
```bash
npm install @kushalbanda/ai
```

TypeBox exports are re-exported from `@kushalbanda/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@kushalbanda/ai';
import { builtinModels } from '@kushalbanda/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@kushalbanda/ai/providers/anthropic';
import { openaiProvider } from '@kushalbanda/ai/providers/openai';
import { openrouterProvider } from '@kushalbanda/ai/providers/openrouter';
import { amazonBedrockProvider } from '@kushalbanda/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@kushalbanda/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@kushalbanda/ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. @kushalbanda/ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@kushalbanda/ai';
...
API-key credentials use the same discriminator as athena's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@kushalbanda/ai';
...
```typescript
import { validateToolCall, type Tool } from '@kushalbanda/ai';
...
```typescript
import { builtinImagesModels } from '@kushalbanda/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@kushalbanda/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@kushalbanda/ai';
import { openAICompletionsApi } from '@kushalbanda/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@kushalbanda/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@kushalbanda/ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are athena thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@kushalbanda/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@kushalbanda/ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for athena-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for athena-controlled thinking values
...
```typescript
import { createModels, type Context } from '@kushalbanda/ai';
import { anthropicProvider } from '@kushalbanda/ai/providers/anthropic';
import { openaiProvider } from '@kushalbanda/ai/providers/openai';
import { googleProvider } from '@kushalbanda/ai/providers/google';
...
```typescript
import { createModels } from '@kushalbanda/ai';
import { anthropicProvider } from '@kushalbanda/ai/providers/anthropic';
...
- `@kushalbanda/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@kushalbanda/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@kushalbanda/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@kushalbanda/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@kushalbanda/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@kushalbanda/ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @kushalbanda/ai login              # interactive provider selection
npx @kushalbanda/ai login anthropic    # login to specific provider
npx @kushalbanda/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@kushalbanda/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@kushalbanda/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@kushalbanda/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@kushalbanda/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@kushalbanda/ai`.
