...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. @athena/ai ships an in-memory default; apps inject persistent storage:
