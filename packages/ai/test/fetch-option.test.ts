...
import { streamSimple as streamAthenaMessages } from "../src/api/athena-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and athena-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamAthenaMessages(createModel("athena-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();
