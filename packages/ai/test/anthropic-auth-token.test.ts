...
describe("Anthropic-compatible user agents", () => {
	it("enforces the athena runtime user agent for Kimi Coding", async () => {
		...
		expect(userAgentHeaders).toEqual([["User-Agent", `athena (${platform()} ${release()}; ${arch()})`]]);
	});

	it("does not apply the athena runtime user agent to Anthropic", async () => {
