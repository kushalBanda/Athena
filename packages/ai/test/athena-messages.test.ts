...
function createModel(baseUrl: string): Model<"athena-messages"> {
	return {
		id: "auto",
		name: "Custom Gateway Auto",
		api: "athena-messages",
...
describe("athena-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		const { baseUrl, requests } = await startServer({
			headers: { "x-athena-gateway-upstream-provider": "anthropic" },
		...
		expect(observedHeaders?.["x-athena-gateway-upstream-provider"]).toBe("anthropic");
	});
...
describe("athena-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("athena-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "athena-messages";
		expect(api).toBe("athena-messages");
	});
});
