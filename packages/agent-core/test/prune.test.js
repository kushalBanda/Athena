...
import { pruneToolOutputs, PRUNE_MINIMUM_GAIN } from "../src/compaction/prune.js";
function toolResultMsg(content) {
    return {
...
describe("pruneToolOutputs", () => {
    ...
    it("prunes old tool results when gain exceeds minimum", () => {
        ...
        const recentMessages = Array.from({ length: 3 }, () => userMsg("y".repeat(1000)));
    ...
    it("does not prune messages within protect window", () => {
        ...
        const messages = Array.from({ length: 5 }, () => toolResultMsg(protectedContent));
        const result = pruneToolOutputs(messages);
        expect(result).toBe(messages);
    });
});
